...
struct Worker {
    ...
    NetworkRequestChannel* workerChannel; // channel for worker threads
};
...
int main(int argc, char * argv[]) {
    ...
    string i = "n"; // default ipc method is FIFO
    ...
    for (int j = 0; j < w; ++j) { // create worker threads
        workers[j].request_buf = &request_buf; // assign arguments
        workers[j].response_buf1 = &request_buf1;
        workers[j].response_buf2 = &request_buf2;
        workers[j].response_buf3 = &request_buf3;
        workers[j].workerChannel = new NetworkRequestChannel(host, port);
        pthread_create(&worker_threads[j], NULL, worker_thread_function, (void *) &workers[j]);
    }
